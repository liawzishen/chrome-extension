...
export const GROWTH_PROFILES = Object.freeze({
  ...
  mature: Object.freeze({
    ...
    crownBranchDepth: TREE_CONFIG.crownBranchDepth,
    rootCount: TREE_CONFIG.rootCount,
    rootDepth: TREE_CONFIG.rootDepth,
    sideBranches: true,
    crownCount: 2,
  }),
});

function getGrowthProfile(stage) {
  return GROWTH_PROFILES[stage] || GROWTH_PROFILES.mature;
}
...
function growRoot(skeleton, start, direction, length, radius, depth, maxDepth, random) {
  const segment = createCurvedSegment(
    start,
    direction,
    length,
...
function generateSkeleton(random, growthProfile) {
  ...
  let trunkLength = growthProfile.trunkSegmentLength;

  for (let level = 0; level < growthProfile.trunkSegments; level += 1) {
    ...
    if (level >= 1 && growthProfile.sideBranches !== false) {
  ...
  for (let crown = 0; crown < (growthProfile.crownCount || 2); crown += 1) {
    ...
    growBranch(
      ...
      trunkRadius,
      0,
      growthProfile.crownBranchDepth,
      random,
    );
  }

  // Roots radiate algorithmically around the base and descend into the void.
  for (let rootIndex = 0; rootIndex < growthProfile.rootCount; rootIndex += 1) {
    const angle = (rootIndex / growthProfile.rootCount) * Math.PI * 2 + randomBetween(random, -0.16, 0.16);
    ...
    growRoot(
      ...
      base,
      rootDirection,
      randomBetween(random, 0.7, 0.94),
      0.17,
      0,
      growthProfile.rootDepth,
      random,
    );
  }
...
function assignChaptersToTerminals(terminals, chapters) {
  if (!chapters.length || !terminals.length) return new Map();
...
/**
 * Create stable metadata anchors for every saved note without forcing the
 * botanical grammar to grow one physical terminal per record. The anchors are
 * selected from real leaves, so they remain attached to the visible canopy.
 * If a chapter eventually contains more notes than leaves, deterministic
 * offsets keep every note addressable instead of dropping labels.
 */
function createChapterAnchors(leaves, chapters) {
  if (!chapters.length || !leaves.length) return [];
  const selectedIndexes = [];
  const availableCount = Math.min(leaves.length, chapters.length);
  let firstIndex = 0;
  for (let index = 1; index < leaves.length; index += 1) {
    if (leaves[index].center.y > leaves[firstIndex].center.y) firstIndex = index;
  }
  selectedIndexes.push(firstIndex);
  while (selectedIndexes.length < availableCount) {
    let bestIndex = -1;
    let bestDistance = -Infinity;
    for (let index = 0; index < leaves.length; index += 1) {
      if (selectedIndexes.includes(index)) continue;
      const candidate = leaves[index].center;
      const nearestDistance = Math.min(...selectedIndexes.map((selectedIndex) => {
        const chosen = leaves[selectedIndex].center;
        const dx = candidate.x - chosen.x;
        const dy = candidate.y - chosen.y;
        const dz = candidate.z - chosen.z;
        return dx * dx + dy * dy + dz * dz * 0.28;
      }));
  ...
  selectedIndexes.sort((first, second) => leaves[second].center.y - leaves[first].center.y);
  return chapters.map((chapter, index) => {
    const source = leaves[selectedIndexes[index % selectedIndexes.length]];
    const layer = Math.floor(index / selectedIndexes.length);
    const angle = index * Math.PI * (3 - Math.sqrt(5));
    const offset = layer > 0
      ? new THREE.Vector3(Math.cos(angle), 0.22, Math.sin(angle)).multiplyScalar(0.08 * layer)
      : new THREE.Vector3();
    return {
      ...source,
      chapter,
      center: source.center.clone().add(offset),
      labelAnchor: source.labelAnchor.clone().add(offset),
    };
  });
}
...
export function createParticleTree(chapters, renderer, options = {}) {
  ...
  const growthStage = options.growthStage === 'seedling' || options.growthStage === 'growing'
    ? options.growthStage
    : 'mature';
