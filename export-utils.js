(function attachExamCramExport(global) {
  "use strict";

  const MODEL_SCHEMA = "exam-cram-export-v1";
  ...
  function buildExportBody(input, selectionValue) {
    ...
    return `<article class="study-document">
      <header class="document-header"><span class="brand">Exam-Cram Study Export</span><h1>${escapeHtml(model.title)}</h1><p class="document-meta">${escapeHtml(meta)}</p>${model.source.title ? `<p><strong>Source:</strong> ${escapeHtml(model.source.title)}${model.source.hostname ? ` · ${escapeHtml(model.source.hostname)}` : ""}</p>` : ""}${model.source.chapter ? `<p><strong>Saved to:</strong> ${escapeHtml(model.source.chapter)}</p>` : ""}</header>
      ...
      <footer>Generated by Exam-Cram. Check AI-generated study material against the original source.</footer>
    </article>`;
  }
  ...
  function resolveVendors(override) {
    ...
    if (global.ExamCramExportVendor?.docx && global.ExamCramExportVendor?.pdfLib) return global.ExamCramExportVendor;
    if (typeof require === "function") {
      return { docx: require("docx"), pdfLib: require("pdf-lib") };
    }
    throw new Error("Export libraries are unavailable. Rebuild export-vendor.bundle.js and reload Exam-Cram.");
  }
  ...
  async function createDocxBytes(input, selectionValue, vendorOverride) {
    ...
    const children = [
      new Paragraph({ children: [new TextRun({ text: "EXAM-CRAM STUDY EXPORT", color: "5B55D7", bold: true, size: 18 })] }),
    ...
    children.push(new Paragraph({ spacing: { before: 320 }, children: [new TextRun({ text: "Generated by Exam-Cram. Check AI-generated study material against the original source.", color: "737382", size: 18 })] }));

    const document = new Document({
      creator: "Exam-Cram",
      title: model.title,
      description: "Exam-Cram study export",
  ...
  async function createPdfBytes(input, selectionValue, vendorOverride) {
    ...
    newPage();
    draw("EXAM-CRAM STUDY EXPORT", { size: 9, font: bold, color: colors.accent, after: 8 });
    ...
    draw("Generated by Exam-Cram. Check AI-generated study material against the original source.", { size: 8.5, color: colors.muted, before: 16 });
